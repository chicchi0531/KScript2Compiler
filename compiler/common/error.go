...
const (
	...
	ERR_0005 = "KS0005: インクリメント演算子はint型以外に適用できません。"
	ERR_0006 = "KS0006: デクリメント演算子はint型以外に適用できません。"
	ERR_0007 = "KS0007: not演算子は数値型以外に適用できません。"
	...
	ERR_0012 = "KS0012: 異なる型同士の演算が行われようとしました。"
	...
	ERR_0024 = "KS0024: 関数定義エラー。return文の戻り値の型が定義と一致しません。"
	...
	ERR_0035 = "KS0035: 定義されていない型が使用されました。"
	ERR_0036 = "KS0036: 型定義に失敗しました。循環参照が発生しています。"
	ERR_0037 = "KS0037: 不正な型にメンバアクセスが行われました。"
	ERR_0038 = "KS0038: エントリポイント(main関数)が見つかりません。"
	ERR_0039 = "KS0039: __syscallの戻り値は演算に使用できません。一旦ローカル変数に代入して使用してください。"
	ERR_0040 = "KS0040: 初期化の代入に使用できるのは、=演算子のみです。"
	ERR_0041 = "KS0041: システムコールは型指定のない変数に代入できません。型を明示する必要があります。"
	ERR_0042 = "KS0042: 構造体同士の演算はサポートしていません"
	ERR_0043 = "KS0043: 配列同士の演算はサポートしていません。"
	ERR_0044 = "KS0044: 小数点演算でサポートされていない演算です。"
	ERR_0045 = "KS0045: 右辺がVoid型のため、左辺を初期化できませんでした。"
)
